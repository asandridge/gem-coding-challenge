                    ...
                    <div>
                        {props.quizData[question].answers.map((answer, idx) => (
                            <div key={question+idx}>
                                <p style={{ marginLeft: '5%', color: getColor(question, idx), fontWeight: props.quizData[question].correctAnswer === idx ? "bold" : "normal" }}>{answer}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ))}
            <Button onClick={() => {props.onSubmit()}} variant="contained" style={{ color: "white", width: "15%" }}>Try Again</Button>
        </div>
    );
}
